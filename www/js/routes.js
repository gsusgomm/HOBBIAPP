var routes = [
  ...
  {
    path: '/cte_detalle/',
    url: './pages/cte_detalle.html',
    on: {
        pageInit: function (event, page){
            /*if (isLogged()){
                facebookLogged();
                twitterLogged();
                $('#smPuntos').html(window.localStorage["points"] + ' / 2500 puntos')
                var p = ((window.localStorage["points"] * 100) / 2500);
                $('.GaugeMeter').data('percent', Math.ceil(p));
                $('.GaugeMeter').gaugeMeter();
                setTimeout(function(){mainfeed()}, 500);
            }*/
        }
    }
  },
